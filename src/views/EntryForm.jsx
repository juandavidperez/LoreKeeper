    ...
    <div className="flex flex-col gap-6 animate-fade-in pb-24 h-full">
      ...
      <div className="bg-zinc-900 border border-zinc-800 p-5 rounded-xl grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="flex flex-col gap-1.5">
          <label htmlFor="entry-book" className="text-[10px] uppercase tracking-widest text-zinc-500 ml-1">Libro Actual</label>
          <select
            id="entry-book"
        ...
        <div className="flex flex-col gap-1.5">
          <label htmlFor="entry-chapter" className="text-[10px] uppercase tracking-widest text-zinc-500 ml-1">Ubicación (Cap/Pág)</label>
          <input
            id="entry-chapter"
      ...
      <div className="bg-zinc-900 border border-amber-500/30 p-5 rounded-xl shadow-2xl">
        <div className="flex justify-between items-center mb-2">
          <label htmlFor="entry-reingreso" className="text-[10px] font-bold uppercase tracking-widest text-amber-500">Resumen de Reingreso</label>
          {speechSupported && (
            <button
        ...
        {speechError && (
          <p className="text-[10px] text-red-400 mb-2 italic">{speechError}</p>
        )}
        <textarea
          id="entry-reingreso"
        ...
        >
          {form.characters.map((c) => (
            <div key={c.id} className="bg-zinc-950 p-4 rounded-lg border border-zinc-800 mb-2 flex flex-col gap-2 relative">
              <button onClick={() => removeItem('characters', c.id)} aria-label="Eliminar" className="absolute top-2 right-2 p-1.5 text-zinc-500 hover:text-red-500"><Trash2 size={14}/></button>
              ...
              <input value={c.tags} onChange={e => updateItem('characters', c.id, 'tags', e.target.value.split(',').map(t => t.trim()).filter(Boolean))} placeholder="Etiquetas (separadas por coma)..." className="bg-transparent text-[10px] text-zinc-500 outline-none" />
        ...
        >
          {form.places.map((p) => (
            <div key={p.id} className="bg-zinc-950 p-4 rounded-lg border border-zinc-800 mb-2 flex flex-col gap-2 relative border-l-2 border-l-cyan-500/40">
              <button onClick={() => removeItem('places', p.id)} aria-label="Eliminar" className="absolute top-2 right-2 p-1.5 text-zinc-500 hover:text-red-500"><Trash2 size={14}/></button>
        ...
        >
          {form.glossary.map((g) => (
            <div key={g.id} className="bg-zinc-950 p-4 rounded-lg border border-zinc-800 mb-2 flex flex-col gap-2 relative border-l-2 border-l-red-500/40">
              <button onClick={() => removeItem('glossary', g.id)} aria-label="Eliminar" className="absolute top-2 right-2 p-1.5 text-zinc-500 hover:text-red-500"><Trash2 size={14}/></button>
        ...
        >
          {form.quotes.map((q, i) => (
            <div key={i} className="bg-zinc-950 p-4 rounded-lg border border-zinc-800 mb-2 flex flex-col gap-2 relative border-l-2 border-l-amber-500/40">
              <button onClick={() => setForm({...form, quotes: form.quotes.filter((_, idx) => idx !== i)})} aria-label="Eliminar" className="absolute top-2 right-2 p-1.5 text-zinc-500 hover:text-red-500"><Trash2 size={14}/></button>
              <textarea
                value={q}
                onChange={e => {
        ...
        >
          {form.worldRules.map((w) => (
            <div key={w.id} className="bg-zinc-950 p-4 rounded-lg border border-zinc-800 mb-2 flex flex-col gap-2 relative border-l-2 border-l-emerald-500/40">
              <button onClick={() => removeItem('worldRules', w.id)} aria-label="Eliminar" className="absolute top-2 right-2 p-1.5 text-zinc-500 hover:text-red-500"><Trash2 size={14}/></button>
              <input value={w.name} onChange={e => updateItem('worldRules', w.id, 'name', e.target.value)} placeholder="Concepto (ej. El Chakra)..." className="bg-transparent border-b border-zinc-800 text-sm font-bold text-amber-100 outline-none pb-1" />
            ...
            <div className="p-4 pt-0 flex flex-col gap-4">
              ...
              {form.connections.map((conn) => (
                <div key={conn.id} className="bg-zinc-950 p-4 rounded-xl border border-zinc-800 flex flex-col gap-3 relative group/conn">
                  <button onClick={() => removeItem('connections', conn.id)} className="absolute top-3 right-3 text-zinc-500 hover:text-red-500 opacity-0 group-hover/conn:opacity-100 transition-all"><Trash2 size={14}/></button>
